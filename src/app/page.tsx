...
import { aggregateStats, cases, clients, pillars, services, stats } from "@/lib/content";
  ...
  return (
    <main>
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 pb-16 pt-10 md:grid-cols-[1.15fr_0.85fr] md:pb-24">
          <div>
            ...
            <p className="mt-6 max-w-xl text-base leading-relaxed text-muted md:text-lg">
              Negócios locais que já cresceram com a Arcano — com neuromarketing como
              diferencial técnico, mais tráfego, conteúdo, branding, IA e sistemas que
              fazem o mês fechar no caixa.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <WhatsAppCta>Quero vender mais este mês</WhatsAppCta>
          ...
          <div className="panel relative overflow-hidden p-4">
            <Image
              ...
              priority
            />
            <p className="mt-3 text-center text-[11px] uppercase tracking-[0.28em] text-gold">
              {site.tagline}
            </p>
          </div>
        </div>
          ...
          <div className="mx-auto grid max-w-6xl grid-cols-2 gap-px md:grid-cols-4">
            {stats.map((item) => (
              <div key={item.label} className="px-5 py-7">
                <p className="font-display text-2xl text-gold-bright md:text-3xl">{item.value}</p>
      ...
      <section className="mx-auto max-w-6xl px-5 py-20">
        <Kicker>Autoridade</Kicker>
        <Title>
          Quatro pilares. <span className="gold-text">Um comercial.</span>
        </Title>
        <p className="mt-4 max-w-2xl text-muted">
          Neuromarketing, tráfego pago, conteúdo e branding — e, na mesma operação,
          IA, posicionamento, vendas online e sistemas personalizados.
        </p>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {services
            .filter((item) => !pillars.some((p) => p.slug === item.slug))
            .map((item) => (
              <Link
                key={item.slug}
                href={`/solucoes/${item.slug}`}
                className="border border-line px-4 py-4 text-sm text-muted hover:border-gold/40 hover:text-cream"
              >
                {item.title}
              </Link>
            ))}
      ...
      <section className="mx-auto max-w-6xl px-5 py-20">
        ...
        <p className="mt-4 max-w-2xl text-muted">
          Painéis, WhatsApp e checkout — a mesma evidência que o empresário local
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-4">
          {aggregateStats.map((item) => (
            <div key={item.label} className="panel p-4">
              ...
              <p className="mt-2 text-[11px] uppercase tracking-[0.14em] text-muted">{item.label}</p>
            </div>
          ))}
        </div>
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          <article className="panel p-6">
            ...
            <p className="mt-3 text-sm leading-relaxed text-muted">
              Neuromarketing aplicado à jornada do cliente local: o que ele sente, o que
              compara e o que o faz fechar no WhatsApp ou no checkout.
            </p>
          </article>
          <article className="panel p-6">
          ...
          <article className="panel p-6">
            <h3 className="font-display text-xl text-cream">Operação completa</h3>
            <p className="mt-3 text-sm leading-relaxed text-muted">
              IA, vendas online e sistemas personalizados (incluindo a lógica de
              despacho e painel que já operamos em produtos como o Vyria) para o
              comercial não morrer no atendimento.
            </p>
          </article>
        </div>
        <div className="mt-8">
